...
1.  Установим программный пакет dplyr.
```{r}
install.packages("dplyr")
```
2. Загрузим библиотеку dplyr
```{r}
library(dplyr)
```
3. Ответим на вопросы

3.1  Сколько строк в датафрейме?
```{r}
starwars %>% nrow()
```
Ответ: 87

3.2  Сколько столбцов в датафрейме?
```{r}
starwars %>% ncol()
```
Ответ: 14

3.3 Как просмотреть примерный вид датафрейма?
```{r}
starwars %>% glimpse()
```
3.4 Сколько уникальных рас персонажей (species) представлено в данных?
```{r}
starwars %>% filter(!is.na(species)) %>% reframe(unique(species))
```
Ответ: 37

3.5  Найти самого высокого персонажа.
```{r}
starwars %>% filter(!is.na(height)) %>% arrange(desc(height)) %>% slice(1) %>% select(name,height)
```
Ответ: Yarael Poof 264

3.6 Найти всех персонажей ниже 170
```{r}
starwars %>% filter(!is.na(height) & height < 170) %>% select(name,height)

```
3.7 Подсчитать ИМТ (индекс массы тела) для всех персонажей. ИМТ подсчитать по
  формуле I = m / h^2 где m - масса, h - рост
```{r}
starwars %>% filter(!is.na(mass) & !is.na(height)) %>% mutate(bmi = mass / (height/100)^2) %>% select(name,bmi)

```
3.8 Найти 10 самых “вытянутых” персонажей. “Вытянутость” оценить по отношению
...
```
3.9  Найти средний возраст персонажей каждой расы вселенной Звездных войн.
```{r}
starwars %>% filter(!is.na(species) & !is.na(birth_year)) %>% group_by(species) %>% summarise(average_age = mean(birth_year, na.rm = TRUE))
```
3.10 Найти самый распространенный цвет глаз персонажей вселенной Звездных
войн.
```{r}
...
3.11 Подсчитать среднюю длину имени в каждой расе вселенной Звездных войн.
```{r}
